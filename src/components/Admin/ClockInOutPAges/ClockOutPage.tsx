...
const ClockOutPage: React.FC = () => {
  ...
  // User info from localStorage (adjust as needed)
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/api/attendances/clock-out", {
        ...
        body: JSON.stringify({
          ...
          // Send current time as clockOut
          clockOut: formattedTime,
          date: new Date().toISOString().split("T")[0], // ISO date yyyy-mm-dd
        }),
      });
      ...
      // Optionally clear storage or redirect user
      localStorage.clear();
      navigate("/login");
    } catch (err) {
